...
public class Tree implements ITree {
    ...
    public void add(String word) {
        ...
        for (char ch : word.toCharArray()) {
            int index = ch - 'a'; // get index of character
            if (current.getChild()[index] == null) { // check if child node exists
    ...
    public ITreeNode find(String word) {
        ...
        for (char ch : word.toCharArray()) {
            int index = ch - 'a';
            if (current.getChild().length <= index || current.getChild()[index] == null) { // check if child node exists
                return null;
            }
            current = current.getChild()[index];
        }
    ...
    private int calcTreeSize(ITreeNode node) {
        ...
        int size = 0;
        for (int i = 0; i < 26; i++) {
            ITreeNode n = node.getChild()[i]; // get child node
            if (n != null)
                size += this.calcTreeSize(n); // recursively calculate size
        }
        return (1 + size); // add 1 for current node
    }
    ...
    public String toString() {
        ...
        this.appendWords(root, words, 0, stringBuilder); // append words to string builder
